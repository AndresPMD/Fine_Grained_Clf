...
__author__ = "Andres Mafla Delgado; Sanket Biswas; Sounak Dey"
__email__ = "amafla@cvc.uab.cat; sbiswas@cvc.uab.cat; sdey@cvc.uab.cat"
...
def test(data_loader, net, cuda, num_classes, batch_size):
    ...
    with torch.no_grad():
        for i, (data, labels, textual_features, image_path) in enumerate(data_loader):
            sample_size = np.shape(data)[0]
            if cuda:
                data, labels, textual_features = data.cuda(), labels.cuda(), textual_features.cuda()
            ...
            if args.dataset == 'context':
                for i in range(0, sample_size):
                    ...
                    with open('/SSD/fine_grained_classification_with_textual_cues/retrieval_features/Context/'+
                              image_name[:-3]+'json','w') as fp:
                        json.dump(features[i].tolist(),fp)

            else:
                for i in range(0, sample_size):
                    image_name = image_path[i].split('/')[-2:]
                    image_name = image_name[0] + '/' + image_name[1]
                    with open('/SSD/fine_grained_classification_with_textual_cues/retrieval_features/Drink_Bottle/' +
                              image_name[:-3] + 'json', 'w') as fp:
                        json.dump(features[i].tolist(), fp)
